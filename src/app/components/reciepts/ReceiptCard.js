...
export default function ReceiptCard({ receipt, onClick }) {
  ...
  const handleMove = async (value) => {
    const folderId = value === "__none" ? null : value || null;
    try {
      await supabase
        .from("receipts")
  ...
  return (
    ...
    >
      <Card
        className="border-0 bg-surface backdrop-blur-sm shadow-lg dark:shadow-gray-500/10 hover:shadow-xl cursor-pointer transition-all duration-300 overflow-hidden p-0 gap-0"
        onClick={onClick}
      >
        <div
        ...
        <div className="p-5">
          ...
          <div className="mt-3">
            ...
            <Select
              value={receipt.folder_id ? String(receipt.folder_id) : "__none"}
